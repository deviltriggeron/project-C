...
START_TEST(input_validation_1) {
  char* sample = "2+2*2";
  int res = validation(sample);
  ck_assert_int_eq(res, SUCCESS);
}
END_TEST

START_TEST(input_validation_2) {
  char* sample = "2^2^3";
  int res = validation(sample);
  ck_assert_int_eq(res, SUCCESS);
}
...
START_TEST(input_validation_10) {
  char* sample = "atan(5)+ln(3)+m(4)+sqrb()";
  int res = validation(sample);
  ck_assert_int_eq(res, FAILURE);
}
END_TEST

START_TEST(input_validation_11) {
  char* sample = ")";
  int res = validation(sample);
  ck_assert_int_eq(res, FAILURE);
}
...
START_TEST(input_validation_12) {
  char* sample = " ";
  int res = validation(sample);
  ck_assert_int_eq(res, FAILURE);
}
...
START_TEST(stack_opt_1) {
  // char *stack
  int is_error = false;
  calc_stack* teststack = create_stack();
  if (teststack != NULL) {
    push(teststack, "I");
    push(teststack, "am");
...
START_TEST(stack_opt_2) {
  // long double* stack
  int is_error = false;
  double_stack* teststack = create_double_stack();
  if (teststack != NULL) {
    push_double(teststack, 5);
    push_double(teststack, 10);
    push_double(teststack, 97);
    ck_assert_int_eq(teststack->length, 3);
    long double top = peek_double(teststack);
    ck_assert_ldouble_eq(top, 97);
    top = pop_double(teststack);
    ck_assert_ldouble_eq(top, 97);
    top = pop_double(teststack);
...
Suite* input_validation_suite(void) {
  ...
  tcase_add_test(tc_core, input_validation_1);
  tcase_add_test(tc_core, input_validation_2);
  tcase_add_test(tc_core, input_validation_3);
  tcase_add_test(tc_core, input_validation_4);
  tcase_add_test(tc_core, input_validation_5);
  tcase_add_test(tc_core, input_validation_6);
  tcase_add_test(tc_core, input_validation_7);
  tcase_add_test(tc_core, input_validation_8);
  tcase_add_test(tc_core, input_validation_9);
  tcase_add_test(tc_core, input_validation_10);
  tcase_add_test(tc_core, input_validation_11);
  tcase_add_test(tc_core, input_validation_12);
